...
// Created: 2/14/2011 5:00:47 PM

function WebTrends(){
	var that=this;
	// begin: user modifiable
	this.dcsid=(function(){var domlist="ocm.knowlysis.com,ocmtest.knowlysis.com,commercetest.ama-assn.org,webt1.ama-assn.org,wwwtest.ama-assn.org,wwwtst.ama-assn.org,extappstest.ama-assn.org,webappstest.ama-assn.org,eprescribingtest.ama-assn.org,ssl3test.ama-assn.org,freidatest.ama-assn.org,searchtest.ama-assn.org,search0test.ama-assn.org,mytest.ama-assn.org,ecfmgtest.ama-assn.org";var doms=that.dcsSplit(domlist);var dlen=doms.length;var host=window.location.hostname.toLowerCase();for (var i=0;i<dlen;i++){if (host==doms[i]){return("dcs3wktuh10000o2ufiaw8tb6_7n2g");}}return("dcsf1dsoi10000kri0ttl6u9x_5s6b");})();
	this.domain="sdc.ama-assn.org";
	this.timezone=-6;
	this.fpcdom=".ama-assn.org";
	this.onsitedoms=(function(){return(window.RegExp?new RegExp(".*ama-assn\.org|.*cme-online\.com|.*addthis\.com|.*commpartners\.com|.*amacodingonline\.com","i"):"");})();
	this.downloadtypes="xls,doc,pdf,txt,csv,zip";
	this.navigationtag="div,table";
	this.adclickparam="WT.ac";
	this.metanames="byline,section,type,Twitter";
...
WebTrends.prototype.amaDelayed=function(){
	var scenario = this.dcsGetCookie("WT.delayed")
	if(scenario!=null) {
		scenario = scenario.split("=");
		this.amaSetProps(scenario);
		document.cookie = 'WT.delayed=; expires=Thu, 01 Jan 1970 00:00:01 GMT; path=/; domain=ama-assn.org';
	}
}

WebTrends.prototype.amaSetProps=function(args){
	var extra = false;
	for (var i=0;i<args.length;i+=2){
		if (args[i].indexOf('WT.')==0){
			if(this.WT[args[i].substring(3)]!=undefined) {
				extra = true;
			}
		} else if (args[i].indexOf('DCS.')==0){
			if(this.DCS[args[i].substring(4)]!=undefined) {
				extra = true;
			}
		} else if (args[i].indexOf('DCSext.')==0){
			if(this.DCSext[args[i].substring(7)]!=undefined) {
				extra = true;
			}
		}
	}
	for (var i=0;i<args.length;i+=2){
		if (args[i].indexOf('WT.')==0){
			this.amaSafeSet(this.WT, args[i].substring(3), args[i+1], extra);
		} else if (args[i].indexOf('DCS.')==0){
			this.amaSafeSet(this.DCS, args[i].substring(4), args[i+1], extra);
		} else if (args[i].indexOf('DCSext.')==0){
			this.amaSafeSet(this.DCSext, args[i].substring(7), args[i+1], extra);
		}
	}
}

WebTrends.prototype.amaSafeSet=function(o, i, v, extra) {
	var c = o[i]==undefined?"":o[i];
	var p = extra?c+";":"";
	o[i] = p + v;
}

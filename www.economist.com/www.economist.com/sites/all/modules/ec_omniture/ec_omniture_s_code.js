...
function s_doPlugins(s) {
  ...
  // get facebook referral value from activity feed
  s.prop16 = s.eVar16 = s.getQueryParam("fb_ref");
  ...
  // If a non-subscriber is viewing an article, let's record their article views history.
  if (s.prop4 == "article" && s.prop13.indexOf('subscriber') == -1) {
    // Count the current article view.
    var article_count = 1;

    // And if there are past article views, count them as well.
    var nidlist = $.cookie('ec_nidlist');
    if (nidlist) {
      nidlist = nidlist.split('|')[0].split(',');
      // If the current article is one of the past ones, "uncount" it.
      // This is so we don't double count that article.
      if ($.inArray(Drupal.settings.nid, nidlist) != -1) {
        article_count = 0;
      }
      article_count += nidlist.length;
    }
    s.prop26 = s.eVar26 = article_count;

    // All non-subscribers are allowed to view 5 free articles.
    var article_allowance = 5;
    // If you have done a full registration, we give you an extra 5.
    if (s.prop13 == 'registered') {
      article_allowance += 5;
    }
    s.prop28 = s.eVar28 = article_allowance;
  }

  // Record the Visual Website Optimizer test/variations for this page.
  if (typeof(vwo_with_variations) == 'function') {
    vwo_with_variations(function(variations, async) {
      if (async) {
        Drupal.omniture.trackClick(true, 'vwo_variations', { 'prop15': variations });
      }
      else {
        s.prop15 = variations;
      }
  ...
  // Record if the user is coming from facebook or twitter.
  if (document.referrer) {
    if (document.referrer.indexOf('facebook.com') >= 0) {
      s.prop36 = s.eVar36 = 'facebook';
    }
    else if (document.referrer.indexOf('twitter.com') >= 0 || s.getQueryParam('fsrc').indexOf('scn/tw') >= 0) {
      s.prop36 = s.eVar36 = 'twitter';
    }
  }
}

...
s.linkDownloadFileTypes="exe,zip,wav,mp3,mov,mpg,avi,wmv,pdf,doc,docx,xls,xlsx,ppt,pptx,mobi,epub";
s.linkInternalFilters="javascript:,fodors.com";
...
/* use this function onClick for partner links; predefined event numbers for each partner */
function linkCode(obj,eventP,linktype) {
	var s=s_gi(s_account);
	s.linkTrackVars='eVar35,events';
	s.linkTrackEvents='event19,event20,event21,event22,event23,event24,event25,event26,event27,event28,event29,event32,event34,event35,event36,event37,event38,event40';
	s.events=eventP;
	s.eVar35=linktype; 
	
	var lt = obj.href != null ? s.lt(obj.href) : "";
	if (lt == "") {
		s.tl(true,'o','form capture');
	}
...
function s_doPlugins(s) {
	...
	s.hbx_lt = "auto";
	s.setupLinkTrack("prop34,prop33,prop35,prop31","SC_LINKS");
	...
	if (s.campaign) {
		s.prop19 = "campaign-" + s.campaign + ":" + s.pageName;
	} else {
		s.prop19 = s.pageName;
	}
	...
	if(s.prop15) {
		...
		s.eVar15=s.prop15;
		var t_search=s.getValOnce(s.eVar15,'ev15',0);
		if(t_search) {
	...
	var d = new Date();
	s.prop26=s.getTimeParting('h','-5',d.getFullYear()); // Set hour 
	s.prop27=s.getTimeParting('d','-5',d.getFullYear()); // Set day
	s.prop28=s.getTimeParting('w','-5',d.getFullYear()); // Set Weekend / Weekday

	/* Copy props to eVars */
	if (s.pageName) {
		if (s.pageName.match("login") || s.pageName.match("newsletter:signup.html") || s.pageName.match("newsletter:subscribe.html")) {
		/* do no reset pagename or section variables in registration or signup */
		} else {
			if (s.pageName && !s.eVar16) {
				s.eVar16=s.pageName;
			}
			if (s.prop20 && !s.eVar20) {
				s.eVar20=s.prop20;
			}
		}
	}
	...
	if (s.prop22 && !s.eVar22) {
		s.eVar22=s.prop22;
	}
	if (s.prop23 && !s.eVar23) {
		s.eVar23=s.prop23;
	}
	if (s.prop24 && !s.eVar24) {
		s.eVar24=s.prop24;
	}
	...
	if (s.prop27 && !s.eVar27) {
		s.eVar27=s.prop27;
	}
	if (s.prop28 && !s.eVar28) {
		s.eVar28=s.prop28;
	}
	...
	s.eVar41=s.prop41=s.channelExtract("/",2,s.href_str,1);
	s.eVar42=s.prop42=s.channelExtract("/",3,s.href_str,1);
	if (s.prop24) {
		s.eVar43=s.prop43=s.prop24;
	}
	/* Set Tracking Codes in Prop44 */
	s.prop44=s.getAndPersistValue(s.campaign,'s_c44_persist',7);
	...
	s.eVar44=s.prop47=s.getAndPersistValue(s.getReferrer(),'visit_referrer',0);
}

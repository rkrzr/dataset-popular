...
(function($) {
  $.widget("js.slides", {
		options: {
			...
			pagination1: true, // [Boolean] Auto generate the pagination
			effects: {
				navigation: "slide",  // [String] Can be either "slide" or "fade"
				pagination1: "slide" // [String] Can be either "slide" or "fade"
			},
			direction: "up", // [String] Define the slide direction: "Up", "Right", "Down", "left"
			fade: {
			...
			startAtSlide: 4, // [Number] What should the first slide be?
...
*/					
			...
			if (this.options.pagination1) {
				this._buildPagination();
				// add current class to first pagination
				this.pagination1.children().eq( this.options.startAtSlide - 1 ).addClass("slidesCurrent");
			}
    ...
    _buildPagination: function() {
			...
			this.slides.each(
				$.proxy(function(index, element) {
					$("<li><a href='#" + index + "' class='slidesNavigation slidesPaginationItem' data-slidesindex=" + index + "> " + ( index + 1 ) + "</a></li>").appendTo(this.pagination1);
				},this)
			);
		...
		next1: function( effect ) {
			this._navigate("next1", effect);
		},
		previous1: function( effect ) {
			this._navigate("previous1", effect);
		},
		slide: function( slide, effect ) {			
			this.element.data("goto", (slide - 1));
			this._navigate("pagination1", effect);
			this.element.removeData("goto");
		},
		_navigate: function( event, effect ) {
			var to, position, direction, next1, prev1, pagination1, $target = $(event.target), currentSlide = this.slides.eq( this.current );
			...
			*/
			...
			if ( $target.hasClass( "slidesNext" ) ) {
			...
			}	else if ( $target.hasClass("slidesPaginationItem") ||  event === "pagination1") {
				...
				effect = effect ? effect : this.options.effects.pagination1;
			}
			...
			if (pagination1) {
				...
				to = next1 ? (this.current + 1) : (prev1 ? this.current - 1 : this.current);
			}
			...
			// creat the loop
			if ( to == this.slides.length && !pagination1 ) {
				// last slide, loop to first
				to = 0;
			} else if ( to == -1 && !pagination1 ) {
			...
			if (this.options.pagination1) {
				...
				this.pagination1.children().removeClass("slidesCurrent");
				this.pagination1.children().eq( to ).addClass("slidesCurrent");
			}
		...
		_slide: function (navigateData) {
			...
			*/
			...
			var isFlipped = navigateData.next1 ? 1 : -1;

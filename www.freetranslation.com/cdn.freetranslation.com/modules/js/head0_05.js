...
function VKI_buildKeyboardInputs(){var self=this;this.VKI_version="1.28";
this.VKI_showVersion=true;this.VKI_target=this.VKI_visible=false;
this.VKI_shift=this.VKI_shiftlock=false;
this.VKI_altgr=this.VKI_altgrlock=false;
this.VKI_dead=false;this.VKI_deadkeysOn=false;
var sel = document.getElementById("param_srcLang");
this.VKI_kt=sel.options[sel.selectedIndex].text;
this.VKI_clearPasswords=false;this.VKI_imageURI="http://www.freetranslation.com/media/keyboard.png";
...
var keylable=document.createElement('label');
keylable.onclick=function(){self.VKI_show(keybut.elem)};
keylable.innerHTML="Special Characters";
keylable.title="Display graphical keyboard interface";
keybut.style.padding="0px";
keybut.style.margin="0px";
keybut.style.marginLeft="10px";
keybut.style.padding="0px";
elem.VKI_attached=true;
if(document.getElementById("keyboardInputInitiator")){
	keybut=document.getElementById("keyboardInputInitiator").appendChild(keybut);
keylable=document.getElementById("keyboardInputInitiator").appendChild(keylable)
}else{
	keylable.style.position="relative";keylable.style.margin="5px";
	keylable.style.marginLeft="10px";keylable.style.color="#005740";
	keylable.style.fontSize="0.9em";
	keylable.style.lineHeight="1em";
	elem.parentNode.insertBefore(keybut,(elem.dir=="rtl")?elem:elem.nextSibling);
	elem.parentNode.insertBefore(keylable,(elem.dir=="rtl")?elem:elem.nextSibling)}
	if(this.VKI_isIE){
		elem.onclick=elem.onselect=elem.onkeyup=function(e){
			...
			for(var x=0,elem;elem=inputElems[x++];)for(var y=0,ex;ex=elem[y++];)if((ex.nodeName=="TEXTAREA"||ex.type=="text"||ex.type=="password")&&ex.className.indexOf("keyboardInput")>-1)this.VKI_attachKeyboard(ex);
			...
			for(ktype in this.VKI_layout){
				if(typeof this.VKI_layout[ktype]=="object"){
					...
					opt.value=ktype;opt.appendChild(document.createTextNode(ktype));
					kblist.appendChild(opt)}}
					if(kblist.options.length){
						kblist.value=this.VKI_kt;
						kblist.onchange=kbchng=function(){
							self.VKI_kt=this.value;
							self.VKI_buildKeys();
							self.VKI_position()};
							th.appendChild(kblist)}
							var label=document.createElement('label');
							label.innerHTML="dead keys";
							var checkbox=document.createElement('input');
							checkbox.type="checkbox";
							checkbox.title="Dead keys: "+((this.VKI_deadkeysOn)?"On":"Off");
							checkbox.defaultChecked=this.VKI_deadkeysOn;
							checkbox.onclick=function(){
								self.VKI_deadkeysOn=this.checked;this.title="Dead keys: "+((this.checked)?"On":"Off");
								self.VKI_modify("");return true};
								label.appendChild(this.VKI_deadkeysElem=checkbox);
								checkbox.checked=this.VKI_deadkeysOn;checkbox.style.display="none";
...
if (typeof this.VKI_layout[this.VKI_kt] == "undefined"){this.VKI_kt = "Common characters"}
...
for(var x=0,hasDeadKey=false,lyt;lyt=this.VKI_layout[this.VKI_kt][x++];){
	...
	for(var y=0,lkey;lkey=lyt[y++];){
		...
		var className=[];
		if(this.VKI_deadkeysOn)for(key in this.VKI_deadkey)if(key===lkey[0]){className.push("alive");break}if(lyt.length>this.VKI_keyCenter&&y==lyt.length)className.push("last");if(lkey[0]==" ")className.push("space");td.className=className.join(" ");
		td.VKI_clickless=0;
		if(!td.click){
			td.click=function(){
				...
				td.onmouseover=function(){
					if(self.VKI_clickless){
						var _self=this;clearTimeout(this.VKI_clickless);
						...
						td.onmouseout=function(){
							...
							td.onmousedown=function(){
								if(self.VKI_clickless)clearTimeout(this.VKI_clickless);if(this.firstChild.nodeValue!="\xa0")this.className+=" pressed"};
 

 
td.onmouseup=function(){if(self.VKI_clickless)clearTimeout(this.VKI_clickless);
 
this.className=this.className.replace(/ ?pressed/g,"")};
 
td.ondblclick=function(){return false};
 
switch(lkey[1]){case"Caps":case"Shift":case"Alt":case"AltGr":case"AltLk":td.onclick=(function(type){return function(){self.VKI_modify(type);
 
return false}})(lkey[1]);
 
break;
 
case"Tab":td.onclick=function(){self.VKI_insert("\t");
 
return false};
 
break;
 
case"Bksp":td.onclick=function(){self.VKI_target.focus();
 
if(self.VKI_target.setSelectionRange){if(self.VKI_target.readOnly&&self.VKI_isWebKit){var rng=[self.VKI_target.selStart||0,self.VKI_target.selEnd||0]}else var rng=[self.VKI_target.selectionStart,self.VKI_target.selectionEnd];
...
if(self.VKI_target.readOnly&&self.VKI_isWebKit){var range=window.getSelection().getRangeAt(0);
 
self.VKI_target.selStart=range.startOffset;
 
self.VKI_target.selEnd=range.endOffset}}else if(self.VKI_target.createTextRange){try{self.VKI_target.range.select()}catch(e){self.VKI_target.range=document.selection.createRange()}if(!self.VKI_target.range.text.length)self.VKI_target.range.moveStart('character',-1);
 
self.VKI_target.range.text=""}else self.VKI_target.value=self.VKI_target.value.substr(0,self.VKI_target.value.length-1);
...
case"Enter":td.onclick=function(){if(self.VKI_target.nodeName!="TEXTAREA"){self.VKI_close();
 
this.className=this.className.replace(/ ?(hover|pressed)/g,"")}else self.VKI_insert("\n");
 
return true};
 
break;
 
default:td.onclick=function(){if(self.VKI_deadkeysOn&&self.VKI_dead){if(self.VKI_dead!=this.firstChild.nodeValue){for(key in self.VKI_deadkey){if(key==self.VKI_dead){if(this.firstChild.nodeValue!=" "){for(var z=0,rezzed=false,dk;
 
dk=self.VKI_deadkey[key][z++];
 
){if(dk[0]==this.firstChild.nodeValue){self.VKI_insert(dk[1]);
 
rezzed=true;
 
break}}}else{self.VKI_insert(self.VKI_dead);
 
rezzed=true}break}}}else rezzed=true}self.VKI_dead=false;
 
if(!rezzed&&this.firstChild.nodeValue!="\xa0"){if(self.VKI_deadkeysOn){for(key in self.VKI_deadkey){if(key==this.firstChild.nodeValue){self.VKI_dead=key;
...
break}}if(!self.VKI_dead)self.VKI_insert(this.firstChild.nodeValue)}else self.VKI_insert(this.firstChild.nodeValue)}self.VKI_modify("");
 
return false}}tr.appendChild(td);
...
z++)if(this.VKI_deadkey[lkey[z]=lkey[z]||"\xa0"])hasDeadKey=true}container.appendChild(table)}this.VKI_deadkeysElem.style.display=(!this.VKI_layout[this.VKI_kt].DDK&&hasDeadKey)?"inline":"none"};
...
case"AltLk":this.VKI_altgrlock=!this.VKI_altgrlock;
 
break;
 
case"Caps":this.VKI_shiftlock=!this.VKI_shiftlock;
...
y++){var className=[],lkey=this.VKI_layout[this.VKI_kt][x][y];
 
if(tds[y].className.indexOf('hover')>-1)className.push("hover");
 
switch(lkey[1]){case"Alt":case"AltGr":if(this.VKI_altgr)className.push("dead");
 
break;
 
case"AltLk":if(this.VKI_altgrlock)className.push("dead");
 
break;
 
case"Shift":if(this.VKI_shift)className.push("dead");
 
break;
 
case"Caps":if(this.VKI_shiftlock)className.push("dead");
 
break;
 
case"Tab":case"Enter":case"Bksp":break;
 
default:if(type)tds[y].firstChild.nodeValue=lkey[vchar];
 
if(this.VKI_deadkeysOn){
	var chart = tds[y].firstChild.nodeValue;
	
	var chart=tds[y].firstChild.nodeValue;
 
if(this.VKI_dead){if(chart==this.VKI_dead)className.push("dead");
 
for(var z=0;
 
z<this.VKI_deadkey[this.VKI_dead].length;
 
z++){if(chart==this.VKI_deadkey[this.VKI_dead][z][0]){className.push("target");
 
break}}}for(key in this.VKI_deadkey)if(key===chart){className.push("alive");
 
break}}}if(y==tds.length-1&&tds.length>this.VKI_keyCenter)className.push("last");
...
if(typeof this.VKI_target.maxlength=="undefined"||this.VKI_target.maxlength<0||this.VKI_target.value.length<this.VKI_target.maxlength){if(this.VKI_target.setSelectionRange){if(this.VKI_target.readOnly&&this.VKI_isWebKit){var rng=[this.VKI_target.selStart||0,this.VKI_target.selEnd||0]}else var rng=[this.VKI_target.selectionStart,this.VKI_target.selectionEnd];
...
if(this.VKI_target.readOnly&&this.VKI_isWebKit){var range=window.getSelection().getRangeAt(0);
 
this.VKI_target.selStart=range.startOffset;
 
this.VKI_target.selEnd=range.endOffset}}else if(this.VKI_target.createTextRange){try{this.VKI_target.range.select()}catch(e){this.VKI_target.range=document.selection.createRange()}this.VKI_target.range.text=text;
 
this.VKI_target.range.collapse(true);
 
this.VKI_target.range.select()}else this.VKI_target.value+=text;
 
if(this.VKI_shift)this.VKI_modify("Shift");
 
if(this.VKI_altgr)this.VKI_modify("AltGr");
 
this.VKI_target.focus()}else if(this.VKI_target.createTextRange&&this.VKI_target.range)this.VKI_target.range.select()};
...
break}}while(elem=elem.offsetParent);
 
if(this.VKI_isIE6)document.body.appendChild(this.VKI_iframe);
 
document.body.appendChild(this.VKI_keyboard);
 
this.VKI_keyboard.style.width="804px";
 

this.VKI_keyboard.style.right=this.VKI_keyboard.style.left="auto";
...
this.VKI_target.focus()}else this.VKI_close()}};
 
this.VKI_position=function(){if(self.VKI_visible){var inputElemPos=VKI_findPos(self.VKI_target);
...
if(window.addEventListener){window.addEventListener('resize',this.VKI_position,false)}
else if(window.attachEvent)window.attachEvent('onresize',this.VKI_position);
 
this.VKI_close=VKI_close=function(){
	if(this.VKI_visible){try{this.VKI_keyboard.parentNode.removeChild(this.VKI_keyboard);
 
if(this.VKI_isIE6)this.VKI_iframe.parentNode.removeChild(this.VKI_iframe)}catch(e){}this.VKI_target.focus();
 
this.VKI_target=this.VKI_visible=false}}};
 
function VKI_findPos(obj){var curleft=curtop=0;
...
return[0,0]}function VKI_scrollDist(){var html=document.getElementsByTagName('html')[0];
...
function VKI_getStyle(obj,styleProp){
	if(obj.currentStyle){
		var y=obj.currentStyle[styleProp]}else if(window.getComputedStyle)var y=window.getComputedStyle(obj,null)[styleProp];
 
return y}function VKI_disableSelection(elem){elem.onselectstart=function(){return false};
